				...
				<table class="table">
					...
					<tbody>
						<c:forEach items="${list}" var="info">
							<tr class="tr_1" onclick="location.href='${pageContext.request.contextPath}/resident/confSelect?confNo=${info.confNo}'">
								<td>${info.confNo}</td>
								<td>${info.confTitle}</td>
								<td><fmt:formatDate value="${info.confDate}" pattern="yy-MM-dd" /></td>
								<td><fmt:formatDate value="${info.confUpdate}" pattern="yy-MM-dd" /></td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
...
/* 	$('.move').on("click", function() {
		...
		actionForm.attr("action", "../../resident/confSelect");
		actionForm.submit();
	}); */
